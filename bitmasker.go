...
//
// The Add method will return a new State mask that includes the original mask
// combined with the given mask m.
//
// The Rem method will return a new State mask that has the original mask with
...
//
package main // import "github.com/kckrinke/bitmasker"
...
func (g *Generator) parsePackageDir(directory string) {
	...
	// TODO: Need to think about constants in test files. Maybe write type_bitmask_test.go
...
func (f *File) genDecl(node ast.Node) bool {
	...
	for _, spec := range decl.Specs {
		...
		for _, name := range vspec.Names {
			...
			if !isUint {
				log.Fatalf("internal error: value of %s is not an unsigned integer: %s", name, value.String())
			}
			v := Value{
				name:   name.Name,
				value:  u64,
				signed: false, // bitmasks are always unsigned integers
				str:    value.String(),
			}
